...
ani_object WrapError(ani_env *env, const std::string &msg)
{
    ...
    if ((status = env->String_NewUTF8(msg.c_str(), msg.size(), &aniMsg)) != ANI_OK) {
        EVENT_LOGE(LOG_TAG_CES, "String_NewUTF8 failed %{public}d", status);
        return nullptr;
    }
    ani_ref undefRef;
    if ((status = env->GetUndefined(&undefRef)) != ANI_OK) {
        EVENT_LOGE(LOG_TAG_CES, "GetUndefined failed %{public}d", status);
        return nullptr;
    }
    ani_class cls = nullptr;
    if ((status = env->FindClass(ERROR_CLASS_NAME, &cls)) != ANI_OK) {
        EVENT_LOGE(LOG_TAG_CES, "FindClass failed %{public}d", status);
        return nullptr;
    }
    ani_method method = nullptr;
    if ((status = env->Class_FindMethod(cls, "<ctor>", "Lstd/core/String;Lescompat/ErrorOptions;:V", &method)) !=
        ANI_OK) {
        EVENT_LOGE(LOG_TAG_CES, "Class_FindMethod failed %{public}d", status);
        return nullptr;
    }
    ani_object obj = nullptr;
    if ((status = env->Object_New(cls, method, &obj, aniMsg, undefRef)) != ANI_OK) {
        EVENT_LOGE(LOG_TAG_CES, "Object_New failed %{public}d", status);
        return nullptr;
    }
    return obj;
}
...
ani_object CreateError(ani_env *env, int32_t code, const std::string &msg)
{
    ...
    if (error == nullptr) {
        EVENT_LOGE(LOG_TAG_CES, "error nulll");
        return nullptr;
    }
    ani_object obj = nullptr;
...
int32_t GetExternalCode(int32_t errCode)
{
    ...
    EVENT_LOGD(LOG_TAG_CES, "GetExternalCode %{public}d to %{public}d", errCode, externalCode);
    return externalCode;
}
...
std::string FindErrorMsg(const int32_t errCode)
{
    ...
    if (findMsg == ERR_CODE_MAP.end()) {
        EVENT_LOGE(LOG_TAG_CES, "FindAnsErrMsg Inner error.");
        return "Inner error.";
    }
